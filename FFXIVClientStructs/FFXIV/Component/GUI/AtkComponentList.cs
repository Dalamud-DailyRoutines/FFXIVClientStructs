...
public unsafe partial struct AtkComponentList : ICreatable<AtkComponentList> {
    ...
    [MemberFunction("48 89 5C 24 ?? 48 89 74 24 ?? 57 48 83 EC ?? 44 0F BF 81 ?? ?? ?? ?? 48 8B F1 0F B7 99 ?? ?? ?? ?? 0F BF FA 44 0F AF C7 66 89 B9 ?? ?? ?? ?? 66 44 89 81 ?? ?? ?? ?? E8 ?? ?? ?? ?? 66 3B DF 74 ?? 48 8B 06 48 8B CE FF 90 ?? ?? ?? ?? 48 8B 5C 24 ?? 48 8B 74 24 ?? 48 83 C4 ?? 5F C3 CC CC CC CC CC CC CC CC CC CC CC CC CC CC CC CC CC CC CC 48 89 5C 24 ?? 57")]
